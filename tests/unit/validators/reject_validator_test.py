...
from validataclass.exceptions import FieldNotAllowedError, ValidationError
...
class RejectValidatorTest:
    ...
    def test_reject_anything_except_none(allow_none, input_data):
        ...
        validator = RejectValidator(**params)
    ...
    def test_custom_errors(error_class, error_code, error_reason, expected_error_code, input_data):
        ...
        # Build expectations
        expected_error_class = FieldNotAllowedError if error_class is UNSET_PARAMETER else error_class
        expected_error_dict = {'code': expected_error_code}
        if error_reason is not None:
            expected_error_dict['reason'] = error_reason

        # Create validator
        if error_class is UNSET_PARAMETER:
            validator = RejectValidator(error_code=error_code, error_reason=error_reason)
        else:
            validator = RejectValidator(error_class=error_class, error_code=error_code, error_reason=error_reason)
