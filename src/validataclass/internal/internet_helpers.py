...
# Internal helper functions intended to be used by `UrlValidator`, `EmailValidator` and potentially other validators
# that work with network host identifiers, i.e. domain names and IP addresses.

def validate_hostname(hostname: str, *, require_tld: bool = False, allow_ip: bool = True) -> bool:
    """
    Checks whether a given string is a valid identifier for a network host, i.e. it is either a valid IP address (only
    if the `allow_ip` option is set) or a valid domain name.

    If `required_tld` is `True`, the domain name must also have an (arbitrary) top level domain to be considered valid,
    e.g. `foo.bar` is always valid, but `foobar` is only valid if `required_tld` is `False`.
    ...
    """
    # First check whether the string looks like an IP address (only contains characters allowed in IP addresses)
